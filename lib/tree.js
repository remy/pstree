...
function find_kids_os390(rootPid, callback) {
  ...
  ps.on('close', () => {
    try {
      ...
      const res = [];
      function populate_kids(ppid) {
        tree.forEach((pair) => {
          if (pair[0] === ppid) {
            res.push(pair[1]);
            populate_kids(pair[1]);
            return;
          }
        });
      }
      populate_kids(rootPid);
      callback(null, res);
    } catch (e) {
      callback(e, null);
    }
...
function find_kids_nix(rootPid, callback) {
...
module.exports = function(rootPid, callback) {
  if (process.platform === 'os390') {
    find_kids_os390(rootPid, callback);
  } else {
    find_kids_nix(rootPid, callback);
  }
}
